...
{
	private static final String JDK="C:\\Program Files\\Java\\jdk1.7.0";
	...
	private static void compile(DecafFile[] files) throws Exception
	{
		...
		{
			long lineNo=diagnostic.getLineNumber()-HEADER_LINES;
			String fileName=diagnostic.getSource().toUri().toString();
					
			if(containsClasses(fileName, files))
			{
				lineNo-=FOOTER_LINES;
			}
			
			System.out.format("Error on line %d in %s%n", lineNo,fileName.replace(".java", ".jdf"));
	...
	private static boolean containsClasses(String fileName, DecafFile[] files)
	{	
		for(DecafFile file: files)
		{
			if(file.getName().equals(fileName.substring(fileName.lastIndexOf('/')+1)))
			{
				return file.containsClasses==true;
			}
		}
